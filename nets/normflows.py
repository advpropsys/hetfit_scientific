...
class GaussianKDE(Distribution):
    ...
    def boundary_distribution(self,x1):
        """
        The function boundary_distribution takes in a value x1 and returns a value y1
        
        :param x1: the x-coordinate of the point on the boundary
        :return: the y1 value.
        """
        y1 = 0.5*x1+0.1
        return y1
    ...
    def sample(self, num_samples):
        ...
        return norm.sample()[norm.sample()[:,0]>self.boundary_distribution(norm.sample()[:,0].mean())]
...
class PlanarFlow(nn.Module):
    ...
    def forward(self, z):
        ...
        preds = z + self.u * self.h(linear_term)
        return preds
...
class NormalizingFlow(nn.Module):
    ...
    def sample(self, base_samples):
        ...
        return samples#[(samples[:,1]!=samples[:,1].mean()+0.1)]
...
def random_normal_samples(n, dim=2):
    ...
    :param dim: the dimension of the space we're sampling from, defaults to 2 (optional)
    :return: A tensor of size n x dim with random normal samples.
    """
